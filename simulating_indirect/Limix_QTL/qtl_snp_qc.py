...
def do_snp_qc(snp_df, min_call_rate, min_maf, min_hwe_P):
    ...
    #Determine MAF.
    genotypeCounter = np.zeros((len(snp_df_c.columns),3), dtype=np.int64)
    ...
    mac = np.zeros((len(snp_df_c.columns)), dtype=np.int64)
    gc = np.zeros((len(snp_df_c.columns)), dtype=np.int64)
    maf = np.zeros((len(snp_df_c.columns)), dtype=np.float64)
    ...
    #Determine HWE.
    hweP = np.zeros((len(snp_df_c.columns)), dtype=np.float64)
...
def do_snp_qc_stringent(snp_df, min_call_rate, min_maf, min_hwe_P, min_hmachR2):
    ...
    machR2 = np.zeros((len(snp_df_c.columns)), dtype=np.float64)
    for snp in range(0, len(snp_df_c.columns)):
        dosageSum = 0.0
        dosageSqrSum = 0.0
