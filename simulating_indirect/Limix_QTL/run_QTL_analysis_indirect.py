...
#Internal code.
import qtl_output_indirect as qtl_output
    ...
    for feature_id in feature_list:
        ...
        ########################################################################################################################################################
            ...
            if ((all(kinship_df.loc[individual_ids,individual_ids].index==sample2individual_feature.loc[phenotype_ds.index]['iid']) if kinship_df is not None else True) &\
                ...
                cov_matrix_direct = np.ones((len(individual_ids), 1))
                cov_matrix_molA = cov_matrix[:, :2]
                cov_matrix_molB = cov_matrix[:, [0, 2]]
                #print("covariate matrixes")
                #print(cov_matrix_direct)
                #print("indirect")
                #print(cov_matrix_molB)
            ...
            if randomeff_mix:
                ...
                lmm = LMM(phenotype, cov_matrix_direct, QS)
                ...
                lmm_indirect_molA.fit(verbose=False)
                lmm_indirect_molB.fit(verbose=False)
            ...
            for snpGroup in utils.chunker(snpQuery, blocksize):
                ...
                alt_lmls_indirect_molA = scannerOut_indirect_molA['lml']
                effsizes_indirect_molA = scannerOut_indirect_molA['effsizes1']
                var_effsizes_se_indirect_molA = scannerOut_indirect_molA['effsizes1_se']
                var_pvalues_indirect_molA = utils.lrt_pvalues(alt_lmls, alt_lmls_indirect_molA)

                scannerOut_indirect_molB = flmm_indirect_molB.fast_scan(G, verbose=False)
                alt_lmls_indirect_molB = scannerOut_indirect_molB['lml']
                effsizes_indirect_molB = scannerOut_indirect_molB['effsizes1']
                var_effsizes_se_indirect_molB = scannerOut_indirect_molB['effsizes1_se']
                var_pvalues_indirect_molB = utils.lrt_pvalues(alt_lmls, alt_lmls_indirect_molB)
