    ...
    for feature_id in feature_list:
        ...
        ########################################################################################################################################################
            ...
            if ((all(kinship_df.loc[individual_ids,individual_ids].index==sample2individual_feature.loc[phenotype_ds.index]['iid']) if kinship_df is not None else True) &\
                ...
                ##addition: adding a column of the molecular state score
                cov_matrix_direct = cov_matrix.iloc[:, 1]
                cov_matrix_molA = cov_matrix.iloc[:, :2]
                cov_matrix_molB = cov_matrix.iloc[:, [0, 2]]
            ...
            if randomeff_mix:
                ...
                lmm_direct = LMM(phenotype, cov_matrix_direct, QS)
                lmm_indirect_molA = LMM(phenotype, cov_matrix_molA, QS)
                lmm_indirect_molB = LMM(phenotype, cov_matrix_molB, QS)
                if not mixed:
                    lmm.delta = 1
                    lmm.fix('delta')
                lmm_direct.fit(verbose=False)
            ...
            if debugger:
                fun_start = time.time()
            null_lml_direct = lmm_direct.lml()
            flmm_direct = lmm_direct.get_fast_scanner()

            null_lml_indirect_molA = lmm_indirect_molA.lml()
            flmm_indirect_molA = lmm_indirect_molA.get_fast_scanner()

            null_lml_indirect_molB = lmm_indirect_molB.lml()
            flmm_indirect_molB = lmm_indirect_molB.get_fast_scanner()
            countChunker = 0
            if debugger:
                fun_end = time.time()
            ...
            for snpGroup in utils.chunker(snpQuery, blocksize):
                ...
                if (not plinkGenotype):
                    ...
                    snp_df_dosage = None
                #We couldmake use of relatedness when imputing.  And impute only based on genetically unique individuals.
                ...
                scannerOut_direct = flmm_direct.fast_scan(G, verbose=False)
                alt_lmls_direct = scannerOut['lml']
                effsizes_direct = scannerOut['effsizes1']
                var_effsizes_se_direct = scannerOut['effsizes1_se']
                var_pvalues_direct = utils.lrt_pvalues(null_lml_direct, alt_lmls_direct)

                scannerOut_indirect_molA = flmm_indirect_molA.fast_scan(G, verbose=False)
                alt_lmls_indirect_molA = scannerOut['lml']
                effsizes_indirect_molA = scannerOut['effsizes1']
                var_effsizes_se_indirect_molA = scannerOut['effsizes1_se']
                var_pvalues_indirect_molA = utils.lrt_pvalues(null_lml_indirect_molA, alt_lmls_indirect_molA)

                scannerOut_indirect_molB = flmm_indirect_molB.fast_scan(G, verbose=False)
                alt_lmls_indirect_molB = scannerOut['lml']
                effsizes_indirect_molB = scannerOut['effsizes1']
                var_effsizes_se_indirect_molB = scannerOut['effsizes1_se']
                var_pvalues_indirect_molB = utils.lrt_pvalues(null_lml_indirect_molB, alt_lmls_indirect_molB)
                
                if debugger:
                    fun_end = time.time()
                    print(" Actual scanning took {}".format(fun_end-fun_start))
                #########################################################################################################################################################
                #adding the covariate term for each simulation


                #########################################################################################################################################################

                #add these results to qtl_results
                temp_df = pd.DataFrame(index = range(len(G_index)),columns=['feature_id','snp_id','direct_p_value','direct_beta','empirical_feature_p_value',
                        'indirect_molA_p_value', 'indirect_molA_effect_size', 'indirect_molB_p_value', 'indirect_molB_effect_size'])
                temp_df['snp_id'] = G_index
                temp_df['feature_id'] = feature_id.replace("/","-")
                temp_df['direct_beta'] = np.asarray(effsizes_direct)
                temp_df['direct_p_value'] = np.asarray(var_pvalues_direct)
                #insert default dummy value
                temp_df['empirical_feature_p_value'] = -1.0
                temp_df['indirect_molA_p_value'] = np.asarray(var_pvalues_indirect_molA)
                temp_df['indirect_molA_effect_size'] = np.asarray(effsizes_indirect_molA)

                temp_df['indirect_molB_p_value'] = np.asarray(var_pvalues_indirect_molB)
                temp_df['indirect_molB_effect_size'] = np.asarray(effsizes_indirect_molB)
...
if __name__=='__main__':
    ...
    covariatefile_w_state = args.covariatefile_w_state
