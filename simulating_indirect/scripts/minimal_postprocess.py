...
def minimal_qtl_processing(QTL_Dir, OutputDir, writeToOneFile=True, compressed = False, overWrite=True, minimalPValue = 1, minimalFeaturePValue = 1, topMode = False, debugMode = False):
    ...
    for file in h5FilesToProcess :
        ...
        data={}
        for key in ['feature_id','snp_id','direct_p_value','direct_beta','empirical_feature_p_value',
        ...
        for ifea,report_feature in enumerate(np.unique(list(frezkeys))):
            ...
            for key in ['snp_id','direct_p_value','direct_beta','empirical_feature_p_value']:
        ...
        ffea['feature_id']=ffea['feature_id'].astype('object')
        #pdb.set_trace()
        ...
        temp['direct_p_value'] = temp['direct_p_value'].astype(float)
        if(minimalPValue<1):
            temp=pd.DataFrame(data).iloc[data['direct_p_value'].astype(float)<minimalPValue]

        if(minimalFeaturePValue<1):
            temp=pd.DataFrame(data).iloc[data['empirical_feature_p_value'].astype(float)<minimalFeaturePValue]

        temp = temp.sort_values(by =['empirical_feature_p_value',"direct_p_value"], ascending=[True,True])
