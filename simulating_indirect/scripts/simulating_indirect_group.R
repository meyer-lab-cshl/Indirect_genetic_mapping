...
sim.sc.A <- sim.sc[, sim.sc$Group == "Group1"]
...
pseudobulk <- aggregateAcrossCells(
  sim.sc.A,
  ids = colData(sim.sc.A)[, c("Sample", "Group")],
...
#                         )
pheno_dataframe <- cbind(
  data.frame("feature_id" = rownames(pseudobulk)),
  data.frame(assay(pseudobulk))
)
#pheno_dataframe$feature_id <- gsub("\\.", "-", pheno_dataframe$feature_id)
colnames(pheno_dataframe) <- c("feature_id", colData(pseudobulk)$Sample)
...
sim.sc.B <- sim.sc[, sim.sc$Group == "Group2"]

# normalize within that group only
sim.sc.B <- logNormCounts(sim.sc.B)


#originally selected 25% of genes to have an indirect effect, changed it to 10%
#first selecting the 25% of genes to adjust in 
genes_to_mod <- sample(rownames(sim.sc.B), size = round(length(sim.sc) * 0.1))
#indirect genes
...
pseudobulk <- aggregateAcrossCells(
  sim.sc.B,
  ids = colData(sim.sc.B)[, c("Sample", "Group")],
  use.assay.type = "logcounts",
  statistics = "mean"
)
...
colnames(agg_g2) <- c("feature_id", colData(pseudobulk)$Sample)
...
effectsize_pathway <- group1_eqtls[group1_eqtls$eSNP.ID == selected_eqtl$eSNP.ID, ]$eQTL.EffectSize
write.table(effectsize_pathway,
            file = paste0(outputdir, "/modelB_selectedsnps_effectsize.txt"),
            quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
